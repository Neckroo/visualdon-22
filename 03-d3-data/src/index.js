...
Promise.all([
    ...
    .then(([users, posts]) => {
        ...
        users.forEach(user => {
            desposts = posts.filter(post => post.userId === user.id)

            d3.select(".container").append("p").text(user.name + " a fait " + desposts.length + " posts")
        });
        ...
        d3.select(".container2")
        .append("p")
        .text(users[postWithLongestId].name+", et un post de "+longestPost+" pitis caractères");
